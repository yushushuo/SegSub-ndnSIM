...
Data::wireEncode(EncodingImpl<TAG>& encoder, bool wantUnsignedPortionOnly) const
{
 ...
 //           ACKtimer
  ...
  //ACKtimer
  totalLength += prependDoubleBlock(encoder,tlv::ACKtimer,getACKtimer());

  //OBU
  totalLength += prependNonNegativeIntegerBlock(encoder,tlv::DataOBU,getOBU());
...
Data::wireDecode(const Block& wire)
{
  ...
  for (++element; element != m_wire.elements_end(); ++element) {
    switch (element->type()) {
      ...
      case tlv::DataOBU: {
        if (lastElement >= 5) {
          NDN_THROW(Error("DataOBU is out of order"));
        }
      ...
      case tlv::ACKtimer: {
        if (lastElement >= 6) {
          NDN_THROW(Error("ACKtimer is out of order"));
        }
        m_acktimer = readDouble(*element);
        lastElement = 6;
        break;
      }
