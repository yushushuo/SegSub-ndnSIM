...
public: // element access
  ...
  int
  getInterestType() const noexcept
  {
      return m_interestType;
  }

  Interest&
  setInterestType(int interestType);

  int
  getLastHopType() const noexcept
  {
      return m_lastHopType;
  }

  Interest&
  setLastHopType(int lastHopType);

  Interest&
  setLocationX(int locationX);

  int
  getLocationX() const noexcept
  {
      return m_location_x;
  }
...
private:
  ...
  int m_interestType; // 1-common 2-sub 3-pre-sub
  int m_lastHopType; // 0-self 1-from_vehicle 2-RSU/ES
  int m_location_x; 
  int m_location_y; 
  int m_speed_x; 
  int m_speed_y;
