...
Interest::wireEncode(EncodingImpl<TAG>& encoder) const
{
  ...
  // OBU
  totalLength += prependNonNegativeIntegerBlock(encoder, tlv::InterestOBU, getOBU());
  // SpeedY
  totalLength += prependNonNegativeIntegerBlock(encoder, tlv::Speed_y, getSpeedY());
  // SpeedX
  totalLength += prependNonNegativeIntegerBlock(encoder, tlv::Speed_x, getSpeedX());
  // LocationY
  totalLength += prependNonNegativeIntegerBlock(encoder, tlv::Location_y, getLocationY());
  // LocationX
  totalLength += prependNonNegativeIntegerBlock(encoder, tlv::Location_x, getLocationX());

  // LastHop-type
  totalLength += prependNonNegativeIntegerBlock(encoder, tlv::LastHopType, getLastHopType());
  
  // Sub-type
  totalLength += prependNonNegativeIntegerBlock(encoder, tlv::InterestType, getInterestType());
...
Interest::wireDecode(const Block& wire)
{
  ...
  for (++element; element != m_wire.elements_end(); ++element) {
    switch (element->type()) {
      ...
      case tlv::InterestType: {
          if (lastElement >= 9) {
              NDN_THROW(Error("InterestType element is out of order"));
          }
          m_interestType = readNonNegativeInteger(*element);
          lastElement = 9;
          break;
      }
      case tlv::LastHopType: {
          if (lastElement >= 10) {
              NDN_THROW(Error("LastHopType element is out of order"));
          }
          m_lastHopType = readNonNegativeInteger(*element);
          lastElement = 10;
          break;
      }
      case tlv::Location_x: {
          if (lastElement >= 11) {
              NDN_THROW(Error("locationX element is out of order"));
          }
          m_location_x = readNonNegativeInteger(*element);
          lastElement = 11;
          break;
      }
      case tlv::Location_y: {
          if (lastElement >= 12) {
              NDN_THROW(Error("locationY element is out of order"));
          }
          m_location_y = readNonNegativeInteger(*element);
          lastElement = 12;
          break;
      }
      case tlv::Speed_x: {
          ...
          m_speed_x = readNonNegativeInteger(*element);
          lastElement = 13;
          break;
      }
      case tlv::Speed_y: {
          ...
          m_speed_y = readNonNegativeInteger(*element);
          lastElement = 14;
          break;
      }
      case tlv::InterestOBU: {
          if (lastElement >= 15) {
              NDN_THROW(Error("OBU element is out of order"));
          }
          m_obu = readNonNegativeInteger(*element);
          lastElement = 15;
          break;
      }
...
Interest&
Interest::setInterestType(int interestType)
{
...
Interest&
Interest::setLastHopType(int lastHopType)
{
    m_lastHopType = lastHopType;
    m_wire.reset();
    return *this;
}

Interest&
Interest::setLocationX(int locationX)
{
    m_location_x = locationX;
    m_wire.reset();
    return *this;
}

Interest&
Interest::setLocationY(int locationY)
{
    m_location_y = locationY;
    m_wire.reset();
    return *this;
}

Interest&
Interest::setSpeedX(int speedX)
{
    m_speed_x = speedX;
    m_wire.reset();
    return *this;
