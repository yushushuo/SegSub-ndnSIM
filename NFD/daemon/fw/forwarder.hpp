...
NFD_PUBLIC_WITH_TESTS_ELSE_PRIVATE: // pipelines
  ...
  */
  ...
  NFD_VIRTUAL_WITH_TESTS bool
  onOutgoingACK(const Data& data, Face& egress);

  /** \brief Content Store hit pipeline for subscription
  */
  NFD_VIRTUAL_WITH_TESTS void
  onSubContentStoreHit(const Interest& interest, const FaceEndpoint& ingress,
                    const shared_ptr<pit::Entry>& pitEntry, const Data& data);
...
private:
  ...
  int
  scheduleMigration(const Interest& interest);

  void
  scheduleCalculatePITsize();//0817pit
...
private:
  ...
  bool m_is_start_pitSize; //0817pit
  scheduler::ScopedEventId m_pitCalEvent;
