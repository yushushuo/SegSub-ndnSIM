...
#ifndef NFD_DAEMON_FW_ES_LOCAL_BEST_ROUTE_STRATEGY_HPP
#define NFD_DAEMON_FW_ES_LOCAL_BEST_ROUTE_STRATEGY_HPP
 ...
 */
class ESLocalBestRouteStrategy : public Strategy
                        , public ProcessNackTraits<ESLocalBestRouteStrategy>
{
public:
  explicit
  ESLocalBestRouteStrategy(Forwarder& forwarder, const Name& name = getStrategyName());

  static const Name&
  getStrategyName();

  //void 
  //prepareSendACK(const Interest& interest, Face& egress);
...
public: // triggers
  ...
  void 
  prepareSendRetACK(const Interest& interest, Face& egress);

  void 
  prepareSendPre(const Interest& interest, int xPre, int yPre);

  double 
  calculateVT2(const Interest& interest);
...
NFD_PUBLIC_WITH_TESTS_ELSE_PRIVATE:
  ...
  static const int rsu_x[25];
  static const int rsu_y[25];
  static const int es_x[6];
  static const int es_y[6];

  int vehicle_Xinfo[30][5];
  int vehicle_Yinfo[30][5];

  friend ProcessNackTraits<ESLocalBestRouteStrategy>;
...
#endif // NFD_DAEMON_FW_ES_LOCAL_BEST_ROUTE_STRATEGY_HPP
