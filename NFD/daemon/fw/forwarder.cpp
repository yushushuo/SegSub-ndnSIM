...
const int Forwarder::rsu_x[25]={0,0,0,0,0,
                    500,500,500,500,500,
                    1000,1000,1000,1000,1000,
                    1500,1500,1500,1500,1500,
                    2000,2000,2000,2000,2000};
const int Forwarder::rsu_y[25]={0,500,1000,1500,2000,
                    0,500,1000,1500,2000,
                    0,500,1000,1500,2000,
                    0,500,1000,1500,2000,
                    0,500,1000,1500,2000};
const int Forwarder::es_x[6]={520,1520,520,1520,520,1520};
const int Forwarder::es_y[6]={250,250,1250,1250,2250,2250};
...
Forwarder::onIncomingInterest(const Interest& interest, const FaceEndpoint& ingress)
{
  ...
  // std::cout<<"InterestType="<<interest.getInterestType()<<std::endl;
  interest.setTag(make_shared<lp::IncomingFaceIdTag>(ingress.face.getId()));
  ++m_counters.nInInterests;

  Name strategyName(m_strategyChoice.findEffectiveStrategy(interest.getName()).getInstanceName());
  name::Component best_route("best-route");
  bool is_vehicle_best_route = false; //
  if(best_route.equals(strategyName.at(-2))){
    is_vehicle_best_route = true;
  }
  if(interest.getInterestType()==2 &&is_vehicle_best_route&& interest.getLastHopType()==1){
    std::cout<<"drop v2v communication"<<std::endl;
    return;
  }
  ...
  if (hasDuplicateNonceInDnl) {
    ...
    //std::cout<<"hasDuplicateNonceInDnl"<<std::endl;
    return;
  }
  ...
  //std::cout<<"PIT SIZE:";
  //std::cout<<m_pit.size()<<std::endl;
  if(!m_is_start_pitSize && interest.getLastHopType()>0){
    m_is_start_pitSize=true;
    this->scheduleCalculatePITsize();
  }
  ...
  if (hasDuplicateNonceInPit) {
    ...
    std::cout<<"hasDuplicateNonceInPit"<<std::endl;
    return;
  }
  ...
  if (!pitEntry->hasInRecords()) {
    this->onContentStoreMiss(interest, ingress, pitEntry);
     ...
     if(interest.getInterestType()==2){
       m_cs.find(interest,
               [=] (const Interest& i, const Data& d) { onSubContentStoreHit(i, ingress, pitEntry, d); },
                              ...
                              const shared_ptr<pit::Entry>& pitEntry)
{
  ...
  //
  Name strategyName(m_strategyChoice.findEffectiveStrategy(*pitEntry).getInstanceName());
  name::Component esLocal("es-local-best-route");
  bool is_es_local = false;
  if(interest.getLastHopType()==2 && esLocal.equals(strategyName.at(-2))){
    is_es_local = true;
    std::cout<<"onContentStoreMiss interest:this is local es"<<std::endl;
  }
  //RetxSuppressionResult suppression;
  // insert in-record
  if(is_es_local && pitEntry->hasInRecords()){ //
    //std::cout<<"es and old Interest"<<std::endl;
    shared_ptr<Interest> temp_interest = make_shared<Interest>(interest);
    temp_interest->setInterestLifetime(0_ms);//
  }
  else if(is_es_local){ 
    shared_ptr<Interest> temp_interest = make_shared<Interest>(interest);
    temp_interest->setInterestLifetime(8000_ms);
  }
  else{
  ...
  //this->setExpiryTimer(pitEntry, 10000_ms);
  ...
  //std::cout<<m_strategyChoice.findEffectiveStrategy(*pitEntry).getInstanceName()<<std::endl;
  ...
  lastExpiring = std::max_element(pitEntry->in_begin(), pitEntry->in_end(),
                                       [] (const auto& a, const auto& b) {
                                         return a.getExpiry() < b.getExpiry();
                                       });
  lastExpiryFromNow = lastExpiring->getExpiry() - time::steady_clock::now();
  this->setExpiryTimer(pitEntry, time::duration_cast<time::milliseconds>(lastExpiryFromNow));
}
                             ...
                             const shared_ptr<pit::Entry>& pitEntry, const Data& data)
{
  ...
  if(!pitEntry->isSubscribeEntry()) pitEntry->isSatisfied = true;
  pitEntry->dataFreshnessPeriod = data.getFreshnessPeriod();

  // set PIT expiry timer to now
  if(!pitEntry->isSubscribeEntry()) this->setExpiryTimer(pitEntry, 0_ms);
  ...
  std::cout<<"onContentStoreHit"<<std::endl;
}
                              ...
                              const shared_ptr<pit::Entry>& pitEntry)
{
  ...
  // send Interest
  if(interest.getLastHopType()==0){
    shared_ptr<Interest> new_interest = make_shared<Interest>(interest);
    new_interest->setLastHopType(1);
    egress.sendInterest(*new_interest);
  }
  else{
...
Forwarder::onIncomingData(const Data& data, const FaceEndpoint& ingress)
{
  ...
  if(!ack.equals(dataName.at(-2))){
    m_cs.insert(data);  //
  }
  bool hasRetransData = false;
  ...
  for (const auto& pitEntry : pitMatches) {
    ...
    if (!pitEntry->isSubscribeEntry() && unsatisfiedDownstreams.empty()) {
      // set PIT expiry timer to now
    ...
    if (!pitEntry->isSubscribeEntry() && !hasRetransData) {
      m_cs.insert(data); //
      hasRetransData = true;
    }
    ...
    Name strategyName(m_strategyChoice.findEffectiveStrategy(*pitEntry).getInstanceName());
    name::Component esDestinated("es-destinated-best-route");
    bool is_es_destinated = false;
    if(esDestinated.equals(strategyName.at(-2))){
      is_es_destinated = true;
    }
    if(is_es_destinated){
      name::Component ack("ack");
      Name dataName(data.getName());
      ...
      m_strategyChoice.findEffectiveStrategy(*pitEntry)
                .afterReceiveData(data, FaceEndpoint(ingress.face, 0), pitEntry);
    }
  ...
  for (const auto& downstream : satisfiedDownstreams) {
    ...
    //std::cout<<"onOutgoingData"<<std::endl;
...
bool
Forwarder::onOutgoingACK(const Data& data, Face& egress)
{
  if (egress.getId() == face::INVALID_FACEID) {
    NFD_LOG_WARN("onOutgoingData out=(invalid) data=" << data.getName());
    return false;
...
Forwarder::onIncomingNack(const lp::Nack& nack, const FaceEndpoint& ingress)
{
  ...
  if (!fw::hasPendingOutRecords(*pitEntry)) {
    //this->setExpiryTimer(pitEntry, 0_ms);//0513
  }
...
Forwarder::onOutgoingNack(const lp::NackHeader& nack, Face& egress,
                          const shared_ptr<pit::Entry>& pitEntry)
{
  std::cout<<"Sending a native NACK in forwarder:";
...
void
Forwarder::scheduleCalculatePITsize()
{
  //std::cout<<"PIT SIZE:";
  //std::cout<<m_pit.size()<<std::endl;
  time::milliseconds pitInterval((long int)(2000));
  m_pitCalEvent = getScheduler().schedule(pitInterval, [=] { scheduleCalculatePITsize(); });
}

void
Forwarder::onSubContentStoreHit(const Interest& interest, const FaceEndpoint& ingress,
                             const shared_ptr<pit::Entry>& pitEntry, const Data& data)
{
  NFD_LOG_DEBUG("onSubContentStoreHit interest=" << interest.getName());
  ++m_counters.nCsHits;
  afterCsHit(interest, data);
  ...
  // dispatch to strategy: after Content Store hit
  m_strategyChoice.findEffectiveStrategy(*pitEntry).afterContentStoreHit(data, ingress, pitEntry);
  //std::cout<<"onContentStoreHit interest:this is local es"<<std::endl;
  //NFD_LOG_DEBUG("onContentStoreMiss interest=" << interest.getName());
  //++m_counters.nCsMisses;
  afterCsMiss(interest);
