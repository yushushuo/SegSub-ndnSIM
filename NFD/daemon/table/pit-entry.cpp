...
bool
Entry::isSubscribeEntry()
{
  if(m_interest->getInterestType()==2){
    return true;
  }
  return false;
}
...
InRecordCollection::iterator
Entry::OnlyinsertInRecord(Face& face, const Interest& interest)
{
  BOOST_ASSERT(this->canMatch(interest));
  bool is_new_incord = false;
...
InRecordCollection::iterator
Entry::UpdateMaxTimerOrinsertInRecord(Face& face, const Interest& interest)
{
  BOOST_ASSERT(this->canMatch(interest));
  bool is_new_incord = false;
